  ...
  return (
    <TooltipProvider>
      <div className="mb-3">
        ...
        {/* Header */}
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center space-y-2 lg:space-y-0 mb-3">
            ...
            <Tooltip>
              ...
              <TooltipContent>
                <p className="text-sm max-w-xs">
                  Real-time pomegranate prices per kilogram from major producing countries. Prices update automatically based on market data feeds and trading patterns.
                </p>
              </TooltipContent>
            </Tooltip>
          </div>
        ...
        {/* 🔄 Dummy Data System: Speculative Live Feed Header */}
        <div className="mb-3 p-3 bg-card border border-border rounded-lg shadow-sm">
          <div className="flex items-center gap-2 mb-2">
            <span className="text-lg">🔄</span>
            <h3 className="text-sm font-semibold text-foreground flex items-center gap-1">
              Dummy Data System: Speculative Live Feed
              <Tooltip>
                <TooltipTrigger><Info className="w-4 h-4 text-muted-foreground hover:text-foreground cursor-help" /></TooltipTrigger>
                <TooltipContent>Simulated real-time price feed for demonstration purposes.</TooltipContent>
              </Tooltip>
            </h3>
          ...
          <div className="text-xs text-muted-foreground mb-2">
            <strong className="text-foreground">Automated Dummy Data Feed (Speculative)</strong> - Backend auto-updates every 30–60 seconds to simulate real activity using basic randomization within set ranges for each country.
          </div>
          
          <div className="flex items-start gap-2">
            <div>
              <p className="text-xs font-medium text-foreground">Speculative Based on Public Data</p>
              <p className="text-xs text-muted-foreground mt-1">
                These prices are estimated using public data, web-scraped figures, and market patterns. This is a simulated ticker for MVP purposes.
              </p>
            </div>
            <Tooltip>
              <TooltipTrigger>
                <Info className="w-3 h-3 text-muted-foreground hover:text-foreground transition-colors cursor-help flex-shrink-0 mt-0.5" />
              </TooltipTrigger>
              <TooltipContent>
                <p className="text-xs max-w-xs">
                  Prices are modeled using public reports, scraped trade listings, and predictive patterns. This is for demonstration purposes.
                </p>
              </TooltipContent>
            </Tooltip>
          </div>
        ...
        <div className="flex items-center gap-2 mb-3">
          <span className="text-sm text-muted-foreground">Live simulation in progress…</span>
        </div>
        ...
        >
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <div className="flex flex-col">
                    <span className="text-2xl font-bold text-foreground">{item.price.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}/kg</span>
                    <span className="text-xs text-muted-foreground">Previous Price: {item.previousPrice.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}</span>
                  </div>
          ...
          {priceData.map((item, index) => (
            <div
              ...
              className="relative flex-shrink-0 w-64 bg-background border border-border rounded-lg p-4 shadow-sm hover:shadow-md transition-all duration-300 hover:scale-105"
            >
              {/* Top Mover Tag for duplicates too */}
              {item.country === topMover.country && (
                <div className="absolute -top-2 -right-2 bg-gradient-to-r from-yellow-400 to-orange-500 text-black text-xs font-bold px-2 py-1 rounded-full shadow-lg animate-pulse">
                  Top Mover
                </div>
